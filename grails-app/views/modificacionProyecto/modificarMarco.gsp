    ...
    <g:form action="guardarMarco" class="marcoLogico">
        ...
        <div class="dialog" title="MATRIZ DE MARCO LOGICO DEL PROYECTO" >
            <div style="float: left;margin: 5px;height: 30px">
                <g:if test="${session.modificacion}">
                    <a href="#" id="btn_editar">Activar edicion</a>
                    <a href="#" id="btn_componentes">Ver Componentes</a>
                </g:if>
                <g:else>
                    <g:link controller="inicio" action="index" class="btn">Salir</g:link>
                </g:else>

            </div>
            <div style="width: 1030px;float: left;border: 1px solid  rgba(145, 192, 95,0.6)" class="ui-corner-all">
                <div class="matriz ui-corner-all campo cmp datos " ml="Fin" div="div_fin" identificador="${fin?.id}">
                    <div class="titulo">Fin</div>
                    <div class="texto agregado ui-corner-all fin" style="min-height: 115px;" id="div_fin" identificador="${fin?.id}">
                        ${fin?.objeto}
                    </div>
                ...
                <div class="matriz ui-corner-all campo cmpDoble " id="div_indi_medios_fin">
                    ...
                    <g:each in="${indicadores}" var="indicador" status="i">
                        ...
                        <div class="matriz ui-corner-all  fila " id="ind" style="${(i==0)?'margin-top:-10px;':''}" >
                            <div class="filaMedio izq ">
                                <div class="texto agregado ui-corner-all fin varios  "  pref="if_" id="if_${indicador.id}" ml="Fin" tipo="1" div="if_${indicador.id}" identificador="${indicador.id}" indicador="${fin.id}">
                                    ${indicador?.descripcion}
                                </div>
                            </div>
                            <div class="filaMedio der">
                                <g:each in="${app.MedioVerificacion.findAllByIndicador(indicador)}" var="med">
                                    <div class="texto agregado ui-corner-all md fin varios"  pref="mf_" id="mf_${med.id}"  ml="Fin"  tipo="2" div="mf_${med.id}" indicador="${indicador.id}" identificador="${med.id}" >${med.descripcion}</div>
                                </g:each>
                                <div class=" texto agregado ui-corner-all md fin varios edicion"  pref="mf_" id="mf_0${i+1}"  ml="Fin" div="mf_0${i+1}" tipo="2"  indicador="${indicador.id}" identificador="0" >Agregar</div>
                            </div>
                        </div>
                    </g:each>

                    <div class="matriz ui-corner-all  fila " id="ind" style="${(band.toInteger()==0)?'margin-top:-10px;':''}" >
                        <div class="filaMedio izq ">
                            <div class="texto agregado ui-corner-all fin varios edicion  " pref="if_" id="if_0" ml="Fin" div="if_0"  tipo="1" identificador="0" indicador="${fin?.id}">
                                Agregar
                            </div>
                        </div>
                        <div class="filaMedio der">
                            <div class=" texto agregado ui-corner-all md fin varios edicion"  pref="mf_" id="mf_0"  ml="Fin" div="mf_0" indicador="0" tipo="2" identificador="0">Agregar</div>
                        </div>
                    </div>
                ...
                <div class="matriz ui-corner-all campo cmp">
                    ...
                    <div class="texto" style=" min-height: 115px;" id="supuestos">
                        <g:each in="${sup}" var="su">
                            <div class="agregado ui-corner-all fin varios" id="sf_${su.id}" ml="Fin" div="sf_${su.id}" identificador="${su.id}" tipo="3" indicador="${fin?.id}">${su.tipo}</div>
                        </g:each>
                        <div class="agregado ui-corner-all md fin varios editar edicion" id="sf_0"  ml="Fin" div="sf_0" identificador="0" tipo="3" indicador="${fin?.id}">Agregar</div>
                    </div>
                </div>

            </div>%{--end del fin--}%

            <div style="width: 1030px;float: left;border: 1px solid  rgba(110, 182, 213,0.6);margin-top: 5px;" class="ui-corner-all">
                <div class="matriz ui-corner-all campoProp cmp datos" ml="Proposito" div="div_prop" identificador="${proposito?.id}">
                    <div class="titulo">Proposito</div>
                    <div class="texto agregado ui-corner-all proposito" style="min-height: 115px;" id="div_prop" identificador="${proposito?.id}">
                        ${proposito?.objeto}
                    </div>
                ...
                <div class="matriz ui-corner-all  cmpDoble  campoProp" id="div_indi_medios_prop" >
                    ...
                    <g:each in="${indiProps}" var="indiProp" status="i">
                        ...
                        <div class="matriz ui-corner-all  fila " id="ind" style="${(i==0)?'margin-top:-10px;':''}" >
                            <div class="filaMedio izq ">
                                <div class="texto agregado ui-corner-all proposito varios"  pref="div_indicador_prop_" id="div_indicador_prop_${indiProp.id}" ml="Proposito"  tipo="1" div="div_indicador_prop_${indiProp.id}" identificador="${indiProp.id}" indicador="${proposito?.id}">
                                    ${indiProp?.descripcion}
                                </div>
                            </div>
                            <div class="filaMedio der">
                                <g:each in="${app.MedioVerificacion.findAllByIndicador(indiProp)}" var="med">
                                    <div class="texto agregado ui-corner-all md proposito varios"  pref="mp_" id="mp_${med.id}" ml="Proposito" div="mp_${med.id}" indicador="${indiProp.id}"  tipo="2" identificador="${med.id}" >${med.descripcion}</div>
                                </g:each>
                                <div class=" texto agregado ui-corner-all md proposito varios edicion"  pref="mp_" id="mp_0${i+1}"  ml="Proposito" div="mp_0${i+1}" tipo="2"  indicador="${indiProp.id}" identificador="0" >Agregar</div>
                            </div>
                        </div>
                    </g:each>
                    ...
                    <div class="matriz ui-corner-all  fila " id="ind" style="${(band.toInteger()==0)?'margin-top:-10px;':''}" >
                        <div class="filaMedio izq ">
                            <div class="texto agregado ui-corner-all proposito varios edicion  "  pref="div_indicador_prop_" id="div_indicador_prop_0" ml="Proposito" div="div_indicador_prop_0" identificador="0" tipo="1" indicador="${proposito?.id}">
                                Agregar
                            </div>
                        </div>
                        <div class="filaMedio der">
                            <div class=" texto agregado ui-corner-all md proposito varios edicion"   pref="mp_" id="mp_0"  ml="Proposito" div="mp_0" indicador="0" tipo="2" identificador="0">Agregar</div>
                        </div>
                    </div>
                ...
                <div class="matriz ui-corner-all campoProp cmp">
                    ...
                    <div class="texto" style=" min-height: 115px;" id="div_sup_prop">
                        <g:each in="${supProp}" var="su">
                            <div class="agregado ui-corner-all proposito varios" id="sp_${su.id}" ml="Proposito" div="sp_${su.id}" identificador="${su.id}" tipo="3" indicador="${proposito?.id}">${su.tipo}</div>
                        </g:each>
                        <div class="agregado ui-corner-all proposito varios editar edicion" id="sp_0" ml="Proposito" div="sp_0" identificador="0" tipo="3" indicador="${proposito?.id}">Agregar</div>
...
//            });
    ...
    $(function() {
        ...
        $("#edicion").val("-1")
        $(".edicion").toggle()
        $("#btn_editar").button().click(function(){
            $(".edicion").toggle()
            if($("#edicion").val()=="-1"){
                $("#edicion").val("1")
                $(this).html("<span class='ui-button-text'>Desactivar edicion</span>")
            }
            else {
                $("#edicion").val("-1")
                $(this).html("<span class='ui-button-text'>Activar edicion</span>")
            }
        });

        $("#btn_componentes").button().click(function(){
        ...
        $("#dlg_combo").dialog( {
            ...
            open:function(){
                $.ajax({
                    type: "POST",
                    url: "${createLink(action:'cargaCombo')}",
                    data:"",
                    success: function(msg) {
                        $("#combo").html(msg)
                    }});
                            ...
                            success: function(msg) {
                                if(msg!="no"){
                                    if($("#c_iden").val()=="0" && $("#c_tipo").val()*1==1){
                                        ...
                                        d1.append(d4)
                                        if($("#c_ml").val()=="Fin"){
                                            d3.addClass("texto agregado ui-corner-all fin varios edicion").attr("pref","div_indicador_").attr("id","div_indicador_0").attr("ml","Fin").attr("div","div_indicador_0").attr("identificador","0").attr("tipo","1")
                                            d5.addClass(" texto agregado ui-corner-all md fin varios edicion").attr("pref","mp_").attr("id","mp_0").attr("ml","Fin").attr("div","mp_0").attr("identificador","0").attr("tipo","2")
                                            $("#div_indi_medios_fin").append(d1)
                                        }else{
                                            d3.addClass("texto agregado ui-corner-all proposito varios edicion").attr("pref","div_indicador_").attr("id","div_indicador_0").attr("ml","Proposito").attr("div","div_indicador_0").attr("identificador","0").attr("tipo","1")
                                            d5.addClass("texto agregado ui-corner-all md proposito varios edicion").attr("pref","mp_").attr("id","mp_0").attr("ml","Proposito").attr("div","mp_0").attr("identificador","0").attr("tipo","2")
