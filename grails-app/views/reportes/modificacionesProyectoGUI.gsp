...
<html>
    <head>
        ...
        <title>Modificaciones proyecto</title>
    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            function reporte(print) {
                var elems = "";
                if ($(".sele").length > 1) {
                    $("#spnAlert").html("Escoja únicamente un proyecto para generar el reporte.");
                ...
                if (elems != "") {
                    if (!print) {
                        var url = "${createLink(action: 'modificacionesProyectoWeb')}?id=" + elems;
                        window.open(url, "_blank");
                    } else {
                        var url = "${createLink(action: 'modificacionesProyectoPDF')}?id=" + elems;
                        location.href = "${createLink(controller:'pdf',action:'pdfLink')}?url=" + url
                    }
                } else {
                    $("#spnAlert").html("Escoja un proyecto para generar el reporte.");
                    $("#dlgAlert").dialog("open");
                    return false;
                }
            }
