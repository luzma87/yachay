...
<div style="margin-left: 10px;">
    <g:link class="btn_arbol" controller="modificacion" action="poaCorrientesMod" id="${unidad.id}">Modificaciones PAPP</g:link>
    ...
    <table style="width: 1000px;">
        ...
        <g:each in="${corrientes}" var="asg" status="i">
            <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                <td colspan="15"><b>Programa:</b>${asg.programa} <b>Actividad:</b> ${asg.actividad} <b>Partida:</b> ${asg.presupuesto.descripcion} <b>Valor:</b>${asg.planificado}
                </td>
            </tr>
            ...
            <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                ...
                <g:each in="${meses}" var="mes">
                    ...
                    <td class="${mes}" style="width: 70px;">
                        <input type="text" class="${mes} valor asg_cor_${asg.id}" mes="${mes}"  value="${g.formatNumber(number: ((progra)?progra.valor:0), format: '###,##0', minFractionDigits: '2', maxFractionDigits: '2')}">
                        <g:set var="suma" value="${suma+progra.valor}"/>
                        <g:if test="${mes==1}">
                            <g:set var="ene" value="${ene.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==2}">
                            <g:set var="feb" value="${feb.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==3}">
                            <g:set var="mar" value="${mar.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==4}">
                            <g:set var="abr" value="${abr.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==5}">
                            <g:set var="may" value="${may.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==6}">
                            <g:set var="jun" value="${jun.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==7}">
                            <g:set var="jul" value="${jul.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==8}">
                            <g:set var="ago" value="${ago.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==9}">
                            <g:set var="sep" value="${sep.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==10}">
                            <g:set var="oct" value="${oct.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==11}">
                            <g:set var="nov" value="${nov.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                        <g:if test="${mes==12}">
                            <g:set var="dic" value="${dic.toDouble()+((progra)?progra.valor:0)}"></g:set>
                        </g:if>
                    </td>
                </g:each>
...
<div id="tabs-2">
    <table style="width: 1000px;">
        ...
        <g:each in="${inversiones}" var="asg" status="i">
            <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                <td colspan="15"><b>Programa:</b>${asg.marcoLogico.proyecto.programa.descripcion} <b>Actividad:</b> ${asg.marcoLogico} <b>Partida:</b> ${asg.presupuesto.descripcion} <b>Valor:</b>${asg.planificado}
                </td>
            </tr>
            ...
            <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                <g:set var="suma" value="${0}"/>
                <g:each in="${meses}" var="mes">
                    <g:set var="progras" value="${ProgramacionAsignacion.findAll('from ProgramacionAsignacion where asignacion = '+asg.id+' and mes = '+mes+' and padre is null')}"></g:set>
                    <g:if test="${progras.size()>0}">
                        <g:set var="progra" value="${progras.pop()}"></g:set>
                    </g:if>
                    <g:else>
                        <g:set var="progra" value="${null}"></g:set>
                    </g:else>
                    <td class="${mes}" style="width: 70px;">
                        <input type="text" class="${mes} valor asg_cor_${asg.id}" mes="${mes}"  value="${g.formatNumber(number: ((progra)?progra.valor:0), format: '###,##0', minFractionDigits: '2', maxFractionDigits: '2')}">
                        <g:if test="${progra}">
...
<script type="text/javascript">
    ...
    $("#anio_asg").change(function () {
        location.href = "${createLink(controller:'modificacion',action:'programacionAsignacionesMod')}?id=${unidad.id}&anio=" + $(this).val()
    });
    ...
    $(".guardar").click(function () {
        ...
        if (total != max) {
            ...
            $.ajax({
                type:"POST",
                url:"${createLink(action:'guardarProgramacionMod',controller:'modificacion')}",
