    ...
    public saveModificacion(nuevo, dominio,usuario, session) {
        ...
        try {
            ...
            if (nuevo.id == null) { //Insert
                ...
                def mod = new app.ModificacionV2()

                if (nuevo.save(flush: false)) {

                    app.ModificacionV2.withTransaction { status ->
                ...
                for (name in names) {
                    ...
                    if (!ignore.contains(name)) {
                        ...
                        def mod = new app.ModificacionV2([usuario:usuario,campo:name,dominio:new org.codehaus.groovy.grails.commons.DefaultGrailsDomainClass(dominio).getName(),id_remoto:old.id.toInteger(),newValue:newValue,oldValue:originalValue,tipo:tipo])
                        listAudt.add(mod)
                        mod =null
...
///////////////////////////////////////////////////// ALERTAS \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    ...
    boolean generarAlerta(from, to, mensaje, controlador, accion, id) {
        try {
            def alerta = new app.alertas.Alerta()
            alerta.from = from
            alerta.usro = to
